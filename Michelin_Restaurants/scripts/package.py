...
issue the validator catches (non-ASCII bytes in outer text, bad color
format, etc.). Auto-bumps version with current UTC timestamp so each
build is treated as a new pack by ForeFlight.
"""
...
ZIP_OUT = ROOT / "Michelin_Restaurants_Pack.zip"
